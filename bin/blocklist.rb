...
DNS_CACHE_FILE = '/var/db/blacklistd.dns.json'
CC_CACHE_FILE = '/var/db/blacklistd.cc.json'
...
load_cache
raw = `blacklistctl dump -b`
# pp raw
raw.split( "\n" ) do |line|
  address_port, _state, _nfail, access = line.split( "\t", 4 )
  next if access == 'last access'
  # p [ address_port, access ]
  ip = address_port.split( '/' ).first.strip
  cc = get_cached_cc( ip )
  dns = get_cached_dns( ip )
  puts "#{address_port}\t#{access}\t#{cc}\t#{dns}"
end
save_cache
